...
object Address extends Address with MongoMetaRecord[Address] {
   ...
   */
  def modify(oid: ObjectId, newShortName: String, newAddressText: String, newCountry: String): Option[Address] = {
   ...
   */
  def modify(oid: ObjectId, newAddress: Address): Option[Address] = {
    modify(oid, newAddress.shortName.get, newAddress.addressText.get, newAddress.country.get)
  }

  /**
   * Add a new address constructed from the parameters unless a matching record is found.
   *
