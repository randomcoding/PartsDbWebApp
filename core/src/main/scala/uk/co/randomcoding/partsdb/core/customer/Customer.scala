 ...
 *
 */
case class Customer(val customerId: Identifier, val customerName: String, val billingAddress: Identifier, val terms: PaymentTerms, val contactDetails: ContactDetails) extends Identifiable {
  override val identifierFieldName = "customerId"
}

object DefaultCustomer extends Customer(DefaultIdentifier, "No Customer", DefaultIdentifier, PaymentTerms(-1), ContactDetails("No Details"))