...
class DocumentRecordTest extends MongoDbTestBase {
  ...
  test("Create doucment with no line items throws expected exception") {
    ...
    val ex = intercept[IllegalArgumentException] {
      Document.create(List.empty[LineItem], DocumentType.Invoice)
    }
  ...
  test("Equality and HashCode of doucments with line items") {
    ...
    val doc1 = Document.create(items, DocumentType.Invoice)
    val doc2 = Document.create(items, DocumentType.Invoice)
    val doc3 = Document.create(items, DocumentType.Invoice)
    ...
    val doc4 = Document.create(items, DocumentType.Order)
  ...
  test("Equality and HashCode of doucments with document numbers") {
    ...
    val doc1 = Document.create(items, DocumentType.Invoice).docNumber(10)
    val doc2 = Document.create(items, DocumentType.Invoice).docNumber(10)
    val doc3 = Document.create(items, DocumentType.Invoice).docNumber(10)
    ...
    val doc4 = Document.create(items, DocumentType.Order).docNumber(10)
    val doc5 = Document.create(items, DocumentType.Invoice).docNumber(20)
  ...
  test("Adding a new Record generates a new record if there is no matching record") {
    val expectedDocument1 = Document.create(Seq(line1), DocumentType.Invoice).docNumber(1l)
    Document.add(Seq(line1), DocumentType.Invoice) should be(Some(expectedDocument1))

    (Document where (_.id exists true) fetch) should be(Seq(expectedDocument1))
    val document2 = Document.create(Seq(line1), DocumentType.Invoice).docNumber(2l)
  ...
  test("Adding a new Record that matches an existing Record returns the matched record and does not add a new record to the database") {
    val document = Document.create(Seq(line2), DocumentType.Invoice).docNumber(1l)
    val copyDocument = Document.create(Seq(line2), DocumentType.Invoice).docNumber(1l)
  ...
  test("Find Matching returns the correct record if the Object Id matches") {
    val document1 = Document.add(Seq(line1), DocumentType.Invoice).get
  ...
  test("Find Matching correctly identifies matches based on object content") {
    val document1 = Document.add(Seq(line2), DocumentType.Invoice).get
    document1.docNumber.get should be(1l)
    val matchingDocument = Document.create(Seq(line2), DocumentType.Invoice).docNumber(1l)

    Document.findMatching(matchingDocument) should be(Some(document1))

    val notMatchingDocument = Document.create(Seq(line1), DocumentType.Invoice).docNumber(10l)
  ...
  test("Removing a Record that exists in the database successfully removes the entry from the database") {
    val document1 = Document.add(Seq(line2), DocumentType.Invoice).get
    val document2 = Document.add(Seq(line1), DocumentType.Invoice).get
