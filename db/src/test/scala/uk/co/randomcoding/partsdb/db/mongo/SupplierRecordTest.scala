...
class SupplierRecordTest extends MongoDbTestBase {
  ...
  test("Equality and HashCode") {
    val s1 = create("Supplier 1", contactDave, Seq(partCost1))
    val s2 = create("Supplier 1", contactDave, Seq(partCost1))
    val s3 = create("Supplier 1", contactDave, Seq(partCost1))
  ...
  test("Adding a new Record generates a new record if there is no matching record") {
    val s1 = add("Supplier 1", contactDave, Seq(partCost1)).get

    allSuppliers should be(List(s1))

    val s2 = add("Supplier 2", contactSally, Seq(partCost2)).get
  ...
  test("Adding a new Record that matches an existing Record returns the matched record and does not add a new record to the database") {
    val s1 = add("Supplier 1", contactDave, Seq(partCost1)).get
    add("Supplier 1", contactDave, Seq(partCost1)) should be(Some(s1))
    allSuppliers should be(List(s1))
  }

  test("Find Matching returns the correct record if the Object Id matches") {
    val s1 = add("Supplier 1", contactDave, Seq(partCost1)).get
    val s2 = Supplier.createRecord.id(s1.id.get).supplierName("Another Supplier").contactDetails(new ObjectId).suppliedParts((1 to 5) map (index => new ObjectId) toList)
  ...
  test("Modify a Record with all new values correctly updates the Record") {
    val s1 = add("Supplier 1", contactDave, Seq(partCost1)).get
    modify(s1.id.get, "Supplier 10", contactSally, Seq(partCost2), "")

    val expectedSupplier = create("Supplier 10", contactSally, Seq(partCost2))
