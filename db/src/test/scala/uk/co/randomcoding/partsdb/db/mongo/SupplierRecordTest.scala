...
class SupplierRecordTest extends MongoDbTestBase {
  ...
  test("Find Matching returns the correct record if the Object Id matches") {
    val s1 = add("Supplier 1", contactDave, address1, Seq(partCost1)).get
    val s2 = Supplier.createRecord.id(s1.id.get).supplierName("Another Supplier").contactDetails(new ObjectId).businessAddress(new ObjectId).suppliedParts(List(PartCost.create(part1, 10.0d, DateTime.now)))
